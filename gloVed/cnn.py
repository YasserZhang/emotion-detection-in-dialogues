...
class TextCNN:
    def __init__(self, sequence_length, num_classes, vocab_size,
        ...
        with tf.device('/cpu:0'), tf.name_scope('embedding'):
            ...
            self.W = tf.Variable(tf.random_uniform([vocab_size, embedding_size], -1, 1), trainable = False, name="W")
            ...
            """
            # expand 1 dimesion at the end of the shape of the tensor
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                #Convolution layer
                filter_shape = [filter_size, embedding_size, 1, num_filters] # 1 is number of channels
                #this is the filter
                W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.1), name = "W")
                b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="b")
                #the output's shape is [batch_size, sequence_length, 1, num_filters]
                ...
                #kernel_size = [batch_size = 1, height = seq - fil + 1, width = 1, num_fil/channel = 1]
                #the shape of output: [batch_size, 1, 1, num_fil]
